...
const observer = new MutationObserver(() => {
  ...
  for (var i = 0; i < svgTags.length; i++) {
    if (svgTags[i].getElementsByTagName('path')[0].getAttribute('d') == searchPath) {
      (svgTags[i] as any).parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.style.opacity = '20%';
